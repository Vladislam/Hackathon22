...
interface UserRepository {
    ...
    class Base(
        ...
        override suspend fun getUserList(query: String): List<User> {
            ...
            return byName.await().toCollection(ArrayList()).apply { addAll(byEmail.await()) }
                .toSet().map { User(it) }.filter { userUid != it.uid }
        }
        ...
        override suspend fun getFriends(): List<User> =
            userDataSource.getCurrentUser().friends.map { scope.async { userDataSource.getUser(it) } }
                .awaitAll().mapNotNull { it }.map { User(it) }
